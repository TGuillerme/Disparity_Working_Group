...
%i) the relationship between diversity and disparity
1 - What is the relation between disparity and diversity? They are decoupled
Disparity can be used as a parallel metric for biodiversity: species diversity (or alpha or beta diversity) aim to describe the number of population as reproductive isolates.
On the other hand, disparity aims to describe the number of traits combination (e.g. shapes, ecologies, etc.).
They can sometimes approximate the same thing (i.e. really low diversity can sometimes mean really low disparity) but have often been showed as decouple [CITES].
A classic example is among mammals, really specious groups like Rodents display low morphological disparity (at least at the superficial scale) but high diversity. Conversely, carnivorans display a relatively lower diversity but a much higher morphological disparity.
...
%iii) parallels with the evolution of gene regulatory evolution, and the extension of this approach to studying the evolution of ecological and functional disparity
...
%do continuous and categorical characters capture the same disparity?
%Does a part represent the whole?
%Do paleontological/ ecological/ functional disparities represent anything more than a disparity of those characters, rather than disparity of morphology/ ecology/ function?
%Time slicing?
%Character equivalence?
%Is ordination meaningful?
...
% It is inevitable that this manuscript is going to be a shit sandwich but I think it is currently an open sandwich and needs an introduction that reviews not only what disparity is, from whence it came and whither it goes, we also need to review what we have learned so far: (, (, ( of (iv) ecological and (v) functional disparity.
 
% I would follow this with a discussion of the problems that have emerged in study: do continuous and categorical characters capture the same disparity? Does a part represent the whole? Do paleontological/ ecological/ functional disparities represent anything more than a disparity of those characters, rather than disparity of morphology/ ecology/ function? Time slicing? Character equivalence? Is ordination meaningful?
 
% I think that I would make this discussion less abstract and use examples. We should also write this as though the prospective articles do not exist – because they don’t. What are the solutions? I think they usually involve forethought in place of playing with data and hoping some interesting but untestable phenomenon falls out. And, of course, follow with a prospect on what could be done in future….
 
% Most of it is there – it just needs jiggling about and editing a bit – as well as adding to.

 
% We were asked originally to write a review on this topic and so we’re pushing the envelope in writing about best practices. I’m comfortable with that, but we still need to achieve the basic expectation of a review – though I always prefer an insightful (more than the sum of its parts) review, rather than a piece of journalism.
 
% So in a round about way, yes, I think we do need to have a ‘what we have learned’ aspect to the manuscript.

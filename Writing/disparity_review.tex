...
At the extremes, clades may be exceptionally species-rich and phenotypically diverse (e.g. cichlids or molluscs), species-rich but phenotypically conservative (e.g. rodents or nematodes), species-poor but phenotypically diverse (e.g. afrotherian mammals), or depauperate in both species and phenotypic diversity (e.g. lungfish).
...
Can anatomy evolve in all ``directions'' or are some anatomies impossible to achieve?
...
\begin{enumerate}
	...
	\item \textbf{Descriptive disparity.} The pioneering studies of disparity characterised the shapes of organisms and how they differed among groups \citep{Foote1995, Briggs1992}.
	...
	\citep[e.g.][]{Fortey1996, Ruta2013, Hopkins2013}.
	This approach has been used to investigate whether some groups are more successful than others in their exploration of new evolutionary strategies \citep{Losos2011}.
	...
	It is particularly common in palaeobiology where it is not possible to directly observe the ecological or functional characteristics of extinct species \citep{Wainwright2005}.

	% \item \textbf{Disparity and evo-devo}
 %    %TG: This section might be linked to "Descriptive disparity" above but Anjali might probably disagree.
 %    The disparity of a group can also give information on a group's evolutionary or developmental constraints.
 %    test hypotheses of developmental constraints,
 %    disparity and integration/modularity


\end{enumerate}

Fundamental insights into evolutionary biology have been elicited from these four types of disparity analysis.
One of the most important insights is the discovery that morphological disparity is often greatest early in the evolutionary history of clades \citep{Foote1997, Erwin2007, Hughes2013}, indicating that the capacity for evolutionary innovation wanes with clade age, which some have argued reflects the evolutionary assembly of gene regulatory networks that constrain later fundamental change  \citep{Erwin2007, Hughes2013}.
...
\noindent Disparity analyses are based on traits, but traits can be characterised in a number of ways:
	...
	2) continuous measurements of features (e.g. lengths) \citep[e.g][]{Friedman2010}; or
	3) more mathematical descriptors from geometric morphometric landmark data (e.g. Procrustes coordinates) \citep[e.g.][]{Cooney2017}, Fourier coefficients \citep[e.g.][]{Foote1995, Spriggs2018} or model-based descriptors \citep[e.g.][]{Raup1961,Saunders2004} (Fig. 2).
None of these approaches are superior, but they may be more or less well-suited to characterising traits under comparison and to the question being asked using those traits \citep{hetherington2015cladistic,Hopkins2017}.
...
Discrete character are commonly recycled from phylogenetic studies \citep[e.g.][]{Brusatte2008,Close2015}.
...
This effect of the anatomical feature used on disparity patterns can be especially challenging where the available data has non-random missing anatomical parts, such as the absence of soft tissue in the fossil record \citep{Deline2018}.
...
% TG: The whole argument here is the same as for logging data in biology: we conserve the same properties of the biological observation (e.g. a distribution of body mass) but we modify it linearly to make it easier to analyse (e.g. the log(body mass)). In my opinion same goes with a data matrix: a distance matrix or a PCA of the observed trait matrix should no modify the biological properties as long as it it changed linearly (if you start arbitrarily removing axis of the PCA or points in the distance matrix, this results in something different). Therefore, morphospace = collected data matrix (any any linear transformation thereof).
...
In practice, this allows us to interpret only the major axis of a highly dimensional dataset as major gradients of biological variation \citep[e.g. the elongation and flattening of birds beaks;][]{Cooney2017}.
...
Although some gradients are easy to detect or interpret (e.g. the elongation and depth of mandibles in fishes on first and second PC axes, respectively; \citealt{Hill2018}), some are not \citep[e.g.][]{Weisbecker2019}.
...
Such non-Euclidean spaces often have non-intuitive properties, for example, straight lines viewed in bivariate plots of some dimensions are not actually straight and, even less intuitively, distances are non-metric \citep[i.e. the distance between A and B is not equal to the distance between B and A][]{Gerber2014}.
Last, but not least, in many cases, ordination might not be necessary.
For example, if an index (e.g. sum or range of variance) used to characterise disparity can use all of the data, it is not necessary to calculate it on the ordinated dataset \citep{Close2015}.
...
\caption{
    ...
    A trait matrix can be transformed into a distance matrix \citep[e.g in][]{Close2015} or an ordinated matrix \citep[e.g. in][]{tyler2011detecting}.
...
\citealt{Diaz2016}, distance from the centroid \citealt{Hopkins2017, Finlay2015-ft}, variance and range \citealt{Brusatte2008}).
...
Finally, if we aim to characterise the density of morphospace occupation (e.g. is group A more closely packed than group B?) indices related to the pairwise distances between elements will be most appropriate (e.g. nearest neighbour distance, pairwise distances, etc. \citealt{Close2015} - see section \ref{section:ordination} above).

In addition to considering what properties of disparity these indices should capture, it is also important to consider the mathematical properties of the indices and their associated caveats \citep{Wills2001, Ciampaglio2001}.
...
However, this is not true of other transformations of the space or when not all dimensions or elements are considered, even in a PCA \citep{Legendre2012}.
...
In spaces with some axes of variance lower than 1, product-based indices used as proxies of volumes (e.g. product of ranges, hypervolume, hypercube, etc.) can tend towards zero fairly quickly for spaces with even a modest number of dimensions \citep{Bellman1966}.
Other types of indices are also extremely sensitive to outliers and can be biased easily by sample size, for example range \citep{Foote1992} or convex hull based indices \citep{Jackson2011}.
...
Thankfully this is becoming easier through the availability of diverse well documented R packages for multidimensional analysis \citep{Navarro2003, Bouxin2005, oksanen2007vegan, Harmon2008, lloyd2016, Guillerme2018b}.
...
Although disparity analyses are now simple to implement in freely available software \citep{Navarro2003, Bouxin2005, oksanen2007vegan, Harmon2008, lloyd2016, Guillerme2018b} it is crucial to remember that they are multidimensional analyses and that multidimensional analyses are complex.

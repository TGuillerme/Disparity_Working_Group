...
Here we deal with some of these issues and present best practice guidelines to allow researchers to optimise their questions % NC: Do we mean questions here or analyses or something else?
 before performing a disparity analysis.
...
For example, if a study focuses on the ecological adaptive radiation of a group, ecological traits are crucial and should be preferred over other kinds of traits, such as those used for species delimitation. % NC: I changed it from evolutionary here to make it clear what an evolutionary trait would be
...
Of course in practice some of these caveats are unavoidable, especially when dealing with the vertebrate fossil record.
    ...
    % \item \textit{Are the data consistent?}
        %NC: Not sure I get this section. What do you mean by consistent???? Also does this fit into the "how much data" section or is it a seaprate consideration?
        %TG: fair enough. This was kind of the miscellaneous section steming from the discussion on data (bullet points about the importance of sexual dimorphism, etc...) 
        % NC: How about adding an "other considerations" subsection to mention all this stuff? Draft below, you may want to edit.     
...
For example, are the observed differences between two groups related to factors unrelated to group designation, e.g. sexual dimorphism?
Are the taxa/groups being compared fairly consistent and truly comparable?
In addition, data can be missing, non-overlapping, hierarchical (e.g. inapplicable), ambiguous, polymorphic or correlated.
...
Although these groups overlap in the two first dimensions (panel 1), they do not overlap between the dimension 1 and 3 thus do no overlap in the multidimensional space at all.
The inverse is of course true as well: if two groups don't overlap between two dimensions (D and C in panel 1), they do not overlap in the multidimensional space, even if they overlap between any other combination of dimensions (dimensions 1 and 3 in panel 2).}} 
...
Reducing the dimensionality through ordination of the data can be advantageous for plotting and visualising the data which can reveal properties of the morphospace not captured by the metrics (see Metrics section \ref{metrics}).
...
%the PCA reflects the number of degrees of freedom (resulting from the superimposition), its geometry is conserved (coordinates remain Euclidean when projected into tangent space and ordinated), etc.%TG: removed below for now.
...
For example, if the disparity metric being used (see below) relies on all the data (see Metric session) it is not necessary to calculate it on ordinated data [e.g. CITE Close].
...
Although some gradients might be easy to detect or interpret (e.g. the elongation of the beak along the first axis), they can often be hard to interpret in morphological gradients terms or simply non-interpretable (e.g. with discrete morphological data, a gradient between the species that have many characters in state 1 and the ones that have more states 0).
